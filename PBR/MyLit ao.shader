    ...
    SubShader
    {
        ...
        Pass
        {
              ...
              float SSS( float3 L, float3 V, float3 N, float3 baseColor)
            {
                ...
                if (NdotL < 0 || NdotV < 0)
                {
                    return (1/PI)*Cdlin * (1-_SSSMetallic);
                }
                ...
                return (1/PI) * lerp(Fd, ss, _subsurface)*Cdlin * (1-_SSSMetallic);
            }
            ...
            half4 frag(Varyings i) : SV_Target
            {
                ...
                //初始化金属度粗糙度
                half4 specGloss = SAMPLE_TEXTURE2D(_MetallicGlossMap, sampler_MetallicGlossMap, i.uv); //获取金属度
                half specGloss2 = SAMPLE_TEXTURE2D(_MetallicGlossMap2,sampler_MetallicGlossMap2,i.uv).r; //获取粗糙度
                half metallic = saturate(specGloss.r * _Metallic);
                half roughness = saturate(max(0.0001, specGloss2) * _Roughness);
                ...
                float3 albedo = saturate(albedoAlpha.rgb * _BaseColor.rgb);
                ...
                */
                ...
                // lambert 漫反射（能量守恒，含 1/PI）
                float3 diffuseBRDF = (albedo * Kd) * (1.0 / PI);
                float3 diffuseColor = diffuseBRDF * (NoL) * lightColor;

                // 可选 SSS 替代兰伯特
                float3 sssTerm = SSS(L, normalize(viewDirWS), normalize(i.normalWS), albedoAlpha.rgb) * Kd;
                float3 directDiffuse = lerp(diffuseColor, sssTerm * (NoL) * lightColor, saturate(_subsurface));
                //迪士尼 Diffuse
                // float FD90 = 0.5 + 2 * VoH * VoH * roughness;
                // float FdV = 1 + (FD90 -1) * pow(1 - NoV, 5);
                // float FdL = 1 + (FD90 -1) * pow(1 - NoL, 5);
                ...
                float3 specularBRDF = (F_Schlick * D_GGX * G_GGX) / max(4.0 * NoV, 1e-4);
                float3 specularColor = specularBRDF * (NoL) * lightColor;

                //间接光 diffuse
                float ao = SAMPLE_TEXTURE2D(_OcclusionMap, sampler_OcclusionMap, i.uv).r;
                float3 indirectDiffuse = 0;
                //lightmap间接光 或 SH 环境光
                #ifdef LIGHTMAP_ON
                    float3 lm = SampleLightmap(i.lightmapUV, i.normalWS);
                    indirectDiffuse = lm * (albedo * Kd);
                #else
                    float3 sh = SampleSH(i.normalWS);
                    indirectDiffuse = sh * (albedo * Kd);
                #endif
                indirectDiffuse *= ao;
                ...
                #ifdef _ADDITIONAL_LIGHTS
                {
                    ...
                    {
                        Light addLight = GetAdditionalLight(li, i.posWS);
                        float3 Ladd = normalize(addLight.direction);
                        float NoLadd = max(0.001, dot(i.normalWS, Ladd));
                        half3 addColor = addLight.color * addLight.distanceAttenuation * addLight.shadowAttenuation;

                        half3 Hadd = normalize(viewDirWS + Ladd);
                        float NoHadd = saturate(dot(i.normalWS, Hadd));
                        float VoHadd = max(0.001, saturate(dot(viewDirWS, Hadd)));

                        float3 Fadd = F0 + (1 - F0) * pow(1 - VoHadd, 5.0);
                        float dAdd = (NoHadd * a2 - NoHadd) * NoHadd + 1;
                        float Dadd = a2 / (PI * dAdd * dAdd);
                        float GVadd = NoV / (NoV * (1.0 - k) + k);
                        float GLadd = NoLadd / (NoLadd * (1.0 - k) + k);
                        float3 specAdd = (Fadd * Dadd * (GVadd * GLadd)) / max(4.0 * NoV, 1e-4);

                        float3 diffAdd = diffuseBRDF * NoLadd * addColor;
                        float3 specColAdd = specAdd * NoLadd * addColor;

                        directDiffuse += diffAdd;
                        specularColor += specColAdd;
                    }
                }
                #endif
                
                float3 color = specularColor + indirectDiffuse + directDiffuse;
